...
def stat_df_open (dir,
  ...
  df['ord_ind'] = df.groupby('var')['ord_ind'].transform(lambda x: x.ffill())
   ...
   # --- объединяем входы и выходы, сохраняя и незакрытые сделки ---
  df_I = df[df['full_i_o']=='I']
  df_O = df[df['full_i_o']=='O']

  # Делая outer-merge, мы получаем и те строки, где нет пары выхода
  full_df = pd.merge(
      df_I,
      df_O,
      on='ord_ind',
  ...
  # Флаг для незакрытых сделок (нет данных по выходу)
  full_df['open_trade'] = full_df['sec_o'].isna()

  # Фильтруем только «валидные» по инструменту:  
  # либо совпадающие sec_i==sec_o, либо незакрытые
  full_df['sec_err'] = np.where(
      (full_df['sec_i'] == full_df['sec_o']) | full_df['open_trade'],
      'ok',
      'err'
  )
